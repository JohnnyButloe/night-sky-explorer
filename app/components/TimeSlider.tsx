...
export default function TimeSlider({
  ...
  const currentMinutes = (currentTime.getTime() - startTime.getTime()) / (60 * 1000)
    ...
    return (
      <div className="w-full space-y-2 text-center text-gray-400">
        ...
        <div className="flex justify-between text-xs text-gray-400">
          <span>{startTime.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) + " PM"}</span>
          <span>{currentTime.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) + " PM"}</span>
          <span>{endTime.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) + " AM"}</span>
        </div>
        ...
        <div className="relative">
          ...
          {celestialObjects.map((object) => (
            <React.Fragment key={object.name}>
              {object.additionalInfo && object.additionalInfo.bestViewingTime && (
                <Tooltip>
              ...
              {object.hourlyData.length > 0 && (
                <Tooltip>
                  ...
                  >
                    <p className="text-xs">
                      Visibility for {object.name}: {object.hourlyData[0].time.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} - {object.hourlyData[object.hourlyData.length - 1].time.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </p>
                  </TooltipContent>
                </Tooltip>
              )}
          ...
          {selectedObject && selectedObject.additionalInfo && selectedObject.additionalInfo.bestViewingTime && (
            <Tooltip>
              <TooltipTrigger asChild>
                <div
