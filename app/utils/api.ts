import type { CelestialData, LocationSuggestion, WeatherData, HourlyForecast } from "../types"
...
export async function getCelestialData(latitude: number, longitude: number, time: string): Promise<CelestialData> {
  if (USE_MOCK_API) {
...
function generateHourlyData(
  ...
  while (currentTime <= endTime) {
    const hoursSinceStart = (currentTime.getTime() - startTime.getTime()) / (60 * 60 * 1000)
    const altitude = baseAltitude + Math.sin((hoursSinceStart * Math.PI) / 12) * 20 // Simulates rising and setting
...
async function getMockCelestialData(latitude: number, longitude: number, time: string): Promise<CelestialData> {
...
async function getRealCelestialData(latitude: number, longitude: number, time: string): Promise<CelestialData> {
  const response = await fetch(`/api/astronomy?latitude=${latitude}&longitude=${longitude}&time=${time}`)
  ...
  const celestialData: CelestialData = {
    objects: data.data.table.rows
      ...
      .map((row: any) => ({
        ...
        altitude: Number.parseFloat(row.cells[0].position.horizontal.altitude.degrees),
        azimuth: Number.parseFloat(row.cells[0].position.horizontal.azimuth.degrees),
        additionalInfo: {
          // This is placeholder data and should be replaced with actual data from the API
          riseDirection: ["East", "Southeast", "Northeast"][Math.floor(Math.random() * 3)],
    ...
    selectedTime: time,
    weather: weatherData,
    sunset: new Date(data.data.observer.sunset).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
    sunrise: new Date(data.data.observer.sunrise).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
  }
...
async function getMockLocationSuggestions(query: string): Promise<LocationSuggestion[]> {
  // Simulating API delay
...
async function getRealLocationSuggestions(query: string): Promise<LocationSuggestion[]> {
