...
export async function GET(request: NextRequest) {
  ...
  if (!latitude || !longitude || !timeOption) {
    return NextResponse.json({ error: "Latitude, longitude, and time are required" }, { status: 400 })
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch celestial data" }, { status: 500 })
  }
}